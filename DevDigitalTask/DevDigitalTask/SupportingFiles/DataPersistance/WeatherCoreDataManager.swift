   ...
   // MARK: - Public properties
    ...
    var getSavedItems: [SavedCity]? {
        ...
        for savedCityEntity in savedCitiesEntities {
            guard let name = savedCityEntity.value(forKey: Dev.CoreData.City.name) as? String,
                  let latitude = savedCityEntity.value(forKey: Dev.CoreData.City.latitude) as? Double,
                  let longitude = savedCityEntity.value(forKey: Dev.CoreData.City.longitude) as? Double else {
                return nil
            }
    ...
    func addNewItem(_ city: String, lat: Double, long: Double) {
        let entity = NSEntityDescription.entity(forEntityName: Dev.CoreData.City.entityName, in: managedContext)!
        let citySavingObject = NSManagedObject(entity: entity, insertInto: managedContext)
        citySavingObject.setValue(city, forKey: Dev.CoreData.City.name)
        citySavingObject.setValue(lat, forKey: Dev.CoreData.City.latitude)
        citySavingObject.setValue(long, forKey: Dev.CoreData.City.longitude)
        let cityEntitiesCount = getManagedObjects()?.count ?? 0
        citySavingObject.setValue(cityEntitiesCount, forKey: Dev.CoreData.City.orderPosition)
        do {
            try managedContext.save()
        } catch let error as NSError {
    ...
    func deleteAll() {
        let fetchRequest: NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: Dev.CoreData.City.entityName)
    ...
    private func getManagedObjects() -> [City]? {
        ...
        let fetchRequest = NSFetchRequest<City>(entityName: Dev.CoreData.City.entityName)
        let sortDescriptor = NSSortDescriptor(key: Dev.CoreData.City.orderPosition, ascending: true)
