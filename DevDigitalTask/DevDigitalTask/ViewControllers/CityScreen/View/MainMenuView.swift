...
class MainMenuView: UIView {
    ...
    private lazy var settingsButton: UIButton = {
        let button = UIButton()
        button.accessibilityIdentifier = Dev.AccessabilityIdentifier.settingsButton
        button.addTarget(self, action: #selector(settingsButtonPressed), for: .touchUpInside)
        let imageConfiguration = UIImage.SymbolConfiguration(scale: .large)
        let image = UIImage(systemName: Dev.SystemImageName.switch2, withConfiguration: imageConfiguration)
    ...
    private lazy var searchButton: UIButton = {
        let button = UIButton()
        button.accessibilityIdentifier = Dev.AccessabilityIdentifier.searchButton
        button.addTarget(self, action: #selector(addNewCityButtonPressed), for: .touchUpInside)
        let imageConfiguration = UIImage.SymbolConfiguration(scale: .large)
        let image = UIImage(systemName: Dev.SystemImageName.magnifyingglass, withConfiguration: imageConfiguration)
        button.setImage(image, for: .normal)
    ...
    var tableView: UITableView = {
        ...
        tableView.register(LoadingCell.self, forCellReuseIdentifier: Dev.CellIdentifier.cityLoadingCell)
        tableView.register(MainMenuTableViewCell.self, forCellReuseIdentifier: Dev.CellIdentifier.cityCell)
